...
df = pd.read_csv("SPE_Rotation.csv")
...
my_expander = st.expander(label='Expand here for the DataSet')
...
# print(df_selection['Theta_rot'])
# if 'z0' in df_selection:
#     print('yes')
...
fig.add_trace(
    go.Line(x = df['Theta_rot'], y = df_selection['z0'], name = "Z = 0")
)
fig.add_trace(
    go.Line(x = df['Theta_rot'], y = df_selection['z1'], name = "Z = 1")
)
fig.add_trace(
    go.Line(x = df['Theta_rot'], y = df_selection['z2'], name = "Z = 2")
)
fig.add_trace(
    go.Line(x = df['Theta_rot'], y = df_selection['z3'], name = "Z = 3")
)
fig.add_trace(
    go.Line(x = df['Theta_rot'], y = df_selection['z4'], name = "Z = 4")
)
fig.add_trace(
    go.Line(x = df['Theta_rot'], y = df_selection['z5'], name = "Z = 5")
)
fig.add_trace(
    go.Line(x = df['Theta_rot'], y = df_selection['z6'], name = "Z = 6")
)
...
st.write('''
### Creating the New Dataset & Plots
''')
...
for i in range(7):
    # print(i)
    # data['Theta'].append(df['Theta_rot'])
    Theta=np.append(Theta,df['Theta_rot'])
# print(Theta)
...
for x in range (1,8):
  ...
  for i in range(18):
    ...
    # data['eV'].append(df[df.columns[x]][i])
      # print(df[df.columns[x]][i])
...
fig2.add_trace(
        go.Scatter(x = X_test['Theta'], y = y_pred, name = d+' Prediction', mode="markers", marker=dict(color="dark blue"))
    )
fig2.add_trace(
...
st.write("## Deploying model for Z = 0")

df_0 = pd.read_csv("Z_0_Rot.csv")
st.write(df_0)
X_0 = df_0[['Theta_rot']]
y_0 = df_0[['z0']]
...
# temp_opt = [0.1,0.15,0.2,0.25,0.3] 
# st.sidebar.subheader("Choose Polynomial regression model paramters")
# s_0 = st.sidebar.select_slider("Choose Test Size (10\% - 30\%):", options = temp_opt, value = 0.2)
...
with st.sidebar:
    st.sidebar.subheader(r"Model Parameters for $Z = 0$")
    deg_0 = st.slider(r'Choose the degree of the polynomial ($0 - 5$):', min_value = 1.0, max_value = 5.0,step = 1.0, value = 2.0)
    deg_0 = int(deg_0)

d_0 = "Degree "+ str(deg_0)
st.write(d_0)
# st.write(X_0_test)
fig3 = go.Figure()
    # plt.scatter(data['Theta'],data['eV']) #Plots the data

poly = make_pipeline(PolynomialFeatures(deg_0), Ridge(alpha=1e-4))
poly.fit(X_0_train,y_0_train) 
y_0_pred = poly.predict(X_0_test)
y_0_pred  = pd.DataFrame(y_0_pred, columns = ['z0_pred'])
col1 , col2, col3 = st.columns(3)
with col3: 
        st.write("Predicted Values:")
        st.write(y_0_pred)
...
with col1:
        st.write('Metrics:')
        st.write('Mean Absolute Error (MAE):', metrics.mean_absolute_error(y_0_test, y_0_pred))
        st.write('Mean Squared Error (MSE):', metrics.mean_squared_error(y_0_test, y_0_pred))
        st.write('Root Mean Squared Error (RMSE):', np.sqrt(metrics.mean_squared_error(y_0_test, y_0_pred)))
fig3.add_trace(
        go.Scatter(x = X_0['Theta_rot'], y = y_0_pred['z0_pred'], name = d_0+' Prediction', mode = 'markers')
    )
fig3.add_trace(
        go.Scatter(x = X_0['Theta_rot'] , y = y_0_test['z0'], name = d_0+ ' Original', mode = 'markers')
    )
fig3.update_xaxes(ticks="outside", tickwidth=2, tickcolor='crimson')
fig3.update_yaxes(ticks="outside", tickwidth=2, tickcolor='crimson')
...
st.write('Co-efficient Data')
coefficient_data = poly[1].coef_
st.write(coefficient_data)
coefficient_names = poly[:-1].get_feature_names_out()
st.write(coefficient_names)

co_eff = pd.DataFrame()
# co_eff[['Feature']] = coefficient_data()
co_eff['Feature'] = coefficient_names
co_eff['Value'] = coefficient_data
st.write(co_eff)
